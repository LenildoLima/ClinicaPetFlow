...
export default function Dashboard() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status?.toLowerCase()) {
      case 'agendado': return <Badge className="badge-info-vivid">Agendado</Badge>;
      case 'em_atendimento': return <Badge className="badge-warning-vivid">Em Atendimento</Badge>;
      case 'concluido': return <Badge className="badge-success-vivid">Concluído</Badge>;
      case 'cancelado': return <Badge className="badge-danger-vivid">Cancelado</Badge>;
      default: return <Badge variant="outline" className="text-secondary-vivid">{status}</Badge>;
    }
  };
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            {/* Consultation Chart */}
            <Card className="rounded-[2rem] border-transparent bg-white/60 backdrop-blur-sm shadow-md transition-all hover:shadow-lg">
              ...
              <CardContent className="h-64">
                <ResponsiveContainer width="100%" height="100%" key={`consultas-chart`}>
                   <AreaChart data={graficoSemana} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                     <defs>
            ...
            <Card className="rounded-[2rem] border-transparent bg-white/60 backdrop-blur-sm shadow-md transition-all hover:shadow-lg">
              ...
              <CardContent className="h-64">
                <ResponsiveContainer width="100%" height="100%" key={`financeiro-${financeiroFilter}`}>
                   <AreaChart data={graficoFinanceiro} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
                     <defs>
          ...
          <section className="grid gap-6 md:grid-cols-[1fr_2fr]">
             <Card className="rounded-[2rem] border-transparent bg-white/60 backdrop-blur-xl shadow-lg border h-full overflow-hidden">
                ...
                <CardContent className="p-0">
                  {proximasConsultas.length === 0 ? (
                    <div className="flex flex-col items-center justify-center p-12 text-center space-y-3">
                       ...
                       <p className="text-slate-400 font-medium italic">Nenhum atendimento registrado para hoje.</p>
                    </div>
                  ) : (
                    <div className="relative pl-8 pr-4 py-4 space-y-6">
                      ...
                      {proximasConsultas.map((c, idx) => (
                        <div key={idx} className="relative group cursor-pointer" onClick={() => navigate('/agenda')}>
                           {/* Dot */}
                           <div className="absolute -left-[1.25rem] top-1.5 w-3 h-3 rounded-full bg-white border-2 border-primary ring-4 ring-white z-10 group-hover:scale-125 transition-transform" />
                           
                           <div className="bg-white p-3 rounded-2xl border border-slate-50 shadow-sm group-hover:shadow-md transition-all group-hover:-translate-y-0.5">
                              <div className="flex justify-between items-start">
                                 <div>
                                   <span className="text-[10px] font-black text-primary bg-primary/10 px-2 py-0.5 rounded-full mb-1 inline-block">{c.horario}</span>
                                   <span className="font-bold text-slate-700 block text-sm">{c.pet_nome}</span>
                                   <p className="text-[10px] text-slate-400 font-medium">Tutor: {c.tutor_nome}</p>
                                 </div>
                  ...
                  <div className="p-4 bg-slate-50/50">
                     <button onClick={() => navigate('/agenda')} className="w-full py-2.5 text-xs font-bold text-primary bg-white border border-slate-100 hover:bg-primary hover:text-white rounded-xl shadow-sm transition-all flex items-center justify-center gap-1">Ver Agenda Completa <ArrowRight className="h-3 w-3" /></button>
                  </div>
                </CardContent>
             </Card>
